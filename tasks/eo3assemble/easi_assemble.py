...
from eodatasets3.validate import validate_dataset, Level
from eodatasets3.properties import Eo3Interface
from eodatasets3.images import GridSpec, MeasurementBundler
from eodatasets3.model import DatasetDoc, ProductDoc, AccessoryDoc


from eodatasets3.validate import ValidationExpectations
...
# Adapted from
# https://github.com/opendatacube/eo-datasets/blob/develop/eodatasets3/assemble.py: DatasetPrepare()
...
class EasiPrepare(Eo3Interface):
    ...
    def _set_output_path(self, output_path):
        ...
        if meta is None:
            ...
            raise RuntimeError(f'Require a local output_path to write to: {output_path}')
    ...
    def measurements(self) -> dict:
        return {name: (grid, path) for grid, name, path in self._measurements.iter_paths()}
    ...
    def _match_measurement_names_to_band_ids(self, mtuples:list, band_ids:dict, supplementary:dict=None) -> dict:
        """
        ...
        for mtuple in mtuples:
            ...
            if len(common) == 1:
                ...
                continue
            raise RuntimeError(f'No unique match for measurements {mtuple} in file paths: {self._dataset_path}')
    ...
    def map_measurements_to_paths(
        ...
        measurement2path = self._match_measurement_names_to_band_ids(mtuples, band_ids, supplementary)
        return measurement2path
    ...
    def relative_to_metadata_path(self, path: Path) -> str:
        ...
        if self._dataset_scheme == 'file':
            ...
            raise ValueError(f'Unsupported path scheme for relative path: {self._dataset_scheme}: {path}')
        return p
    ...
    def to_dataset_doc(
        ...
        if crs is not None:
            ...
            if self.crs:
                warnings.warn(f'Using CRS from measurements and ignoring user input ({self.crs}): {crs}')
        ...
        if validate_correctness:
            ...
            for m in validation_messages:
                if m.level in (Level.info, Level.warning):
                    ...
                    error_count = len([m for m in validation_messages if m.level == Level.error])
                    raise RuntimeError(f'Validation error: {m}, Total errors: {error_count}')
                else:
                    raise RuntimeError(
